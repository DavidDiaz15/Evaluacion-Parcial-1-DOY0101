...
Participar en este proyecto ha sido una experiencia muy diferente, pero importante para comprender la verdadera importancia de la cultura DevOps. Pasamos de la simple creación de una API REST funcional con Spring Boot a gestionar todo su ciclo de vida, automatizando procesos que antes hacíamos manualmente y entendiendo cómo la infraestructura como código cambia por completo la entregar del software.

Uno de los mayores desafíos a los que nos enfrentamos fue lograr la correcta orquestación de todas las herramientas. Configurar el pipeline en GitHub Actions para que no solo construyera la imagen, sino que dependiera de un análisis de calidad estricto con SonarQube y validaciones de seguridad, requirió de mucha prueba y error. Me pareció especialmente interesante y retador la etapa final con la implementación de Istio y CloudWatch, ya que nos demostró lo esencial que resulta tener una observabilidad profunda y un control de red solido cuando los despliegues se realizan en entornos reales como AWS.

Durante el desarrollo, trabajé con David, aportando activamente en la configuración de los entornos, el diseño de los workflows de integración continua y la elaboración de los manifiestos de Kubernetes. También centré mis esfuerzos en la implementación de las políticas de cumplimiento y en la documentación de las evidencias, asegurando que los despliegues cumplieran con los estándares de seguridad exigidos y que los pipelines se bloquearan efectivamente ante cualquier vulnerabilidad crítica.

A nivel personal, este proyecto me deja una gran lección sobre la responsabilidad que implica llevar software a producción. Entendí que escribir el código es solo una parte del trabajo, pero garantizar que este sea seguro, escalable, monitoreable y fácil de mantener es igual de importante. La experiencia de integrar DevSecOps y trabajar de manera colaborativa bajo GitFlow me ha entregado herramientas prácticas y analíticas que sin duda serán invaluables para mi futuro profesional.
